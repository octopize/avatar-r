...
install.packages("httr")  # requires libcurl to work (see above)
```
...
```{r eval=FALSE, include=TRUE}
install.packages("avatar_0.x.x.tar.gz", repos = NULL, type = "source")
```
...
parameters <- list(k=20, column_weights=list(Sepal.Width=3), ncp=3, seed=42)
...
```{r}
result$metrics$hidden_rate
result$metrics$local_cloaking
```
